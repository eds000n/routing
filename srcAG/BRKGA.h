...
public:
	...
	void inject_solution(std::vector<double> sol, int population = 0, int position = 0);
	void multistart();
private:
	// Hyperparameters:
...
template< class Decoder, class RNG >
void BRKGA<Decoder, RNG>::inject_solution(std::vector<double> sol, int population, int position) {
	(*current[population])(position) = sol;
	current[population]->setFitness(position, refDecoder.decode((*current[population])(position)) );
	current[population]->sortFitness();
}

template< class Decoder, class RNG >
void BRKGA<Decoder, RNG>::multistart() {
	std::vector< double > best = getBestChromosome();
	reset();
	inject_solution(best);
}
